...
for i in `seq 4`; do
  ...
  for k in `seq 10`; do
    subdir=MC5-Q1-V10-A${attrNum}/suite${k}

    rm -rf logs/exp_absref/${subdir}
    mkdir -p logs/exp_absref/${subdir}

    for id in `seq ${num}`; do
      (time ./coachecker -t 600 -input ${dataset_dir}/${subdir}/test${id}.aabac) >& logs/exp_absref/${subdir}/output${id}.txt
    done
  done
done
